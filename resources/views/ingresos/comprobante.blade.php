<form action="{{route('ingresos.store')}}" class="flex flex-col p-5 shadow-md w-full mb-5" method="POST">
    @csrf
    <h3 class="text-center py-4">Comprobante {{}}</h3>
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <input type="text" placeholder="dni" class="my-2 border border-slate-600 p-1 w-full" name="dni">
        </div>
        
        <button class="bg-slate-900 text-white w-1/4 mx-auto py-2 my-4" type="button"><a href="{{route('alumnos.crear')}}" class="href">Nuevo</a></button>
    </div>
    <label for="alumno">Alumno encontrado</label>
    <input type="text" name="alumno" class="p-1 border border-slate-600">
 
     <button class="bg-slate-900 text-white  mx-auto py-1 px-2 mt-2" type="button" id="addConcepto">Agregar Concepto</button>
    ...
    <input type="text" disabled name="total" class="w-1/2 mx-auto border border-slate-600 p-1">
